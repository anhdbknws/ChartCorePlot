...
- (void)generateLayOut {
    ...
    plotSpace.yRange                = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromInt(0)
                                                                   length:CPTDecimalFromInt(10 * (int)sets.count)];
    ...
    majorGridLineStyle.lineColor            = [[CPTColor grayColor] colorWithAlphaComponent:0.1];
    ...
    if (fieldEnum == 0) {
        ...
        if (((CPTBarPlot *)plot).barBasesVary) {
            for (NSString *set in [[sets allKeys] sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)]) {
                ...
                offset += [[[data objectForKey:[dates objectAtIndex:idx]] objectForKey:set] doubleValue];
            }
        }
        
        //Y Value
        if (fieldEnum == 1) {
            num = [[[data objectForKey:[dates objectAtIndex:idx]] objectForKey:plot.identifier] doubleValue] + offset;
        }
